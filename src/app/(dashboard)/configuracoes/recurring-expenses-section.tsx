    ...
    <div className="rounded-xl border p-5"
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1">
          <h3 className="text-base font-bold" style={{ color: '#E8F0FE' }}>
          ...
          {items.map(item => {
            ...
            return (
              <div key={item.id}
                ...
                style={{
                  background: item.active ? '#0F1A2B' : 'rgba(15,26,43,.5)',
                  borderColor: '#1E2D45',
                ...
                <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded shrink-0"
                  style={{ background: `${cat?.color ?? '#5A7A9A'}18`, color: cat?.color ?? '#5A7A9A' }}>
                  {cat?.label ?? item.category}
                </span>
                <p className="text-sm font-semibold flex-1 truncate" style={{ color: '#E8F0FE' }}>
                  {item.name}
                </p>
                <p className="text-sm font-bold font-mono shrink-0" style={{ color: item.active ? '#00FF94' : '#5A7A9A' }}>
                  {BRL(item.valueCents)}
                </p>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => handleToggleActive(item)} disabled={pending}
                    ...
                    style={{ color: item.active ? '#FFB800' : '#00FF94' }}
      ...
      {!adding && !editingId && (
        <button onClick={() => setAdding(true)}
          ...
          style={{ borderColor: '#2A3D5C', color: '#00E5FF' }}>
      ...
      {items.length === 0 && (
        <p className="text-[11px] mt-3" style={{ color: '#8AA8C8' }}>
...
function ExpenseForm({
  ...
  return (
    <div className="rounded-lg border-2 p-3 space-y-2"
      style={{ background: '#0F1A2B', borderColor: '#00E5FF' }}>
      ...
      <div className="flex gap-2 items-center">
        <span className="text-xs shrink-0" style={{ color: '#8AA8C8' }}>R$</span>
