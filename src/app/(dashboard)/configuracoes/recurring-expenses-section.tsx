    ...
    <div className="rounded-xl border p-5"
          ...
          {items.map(item => {
            ...
            return (
              <div key={item.id}
                ...
                style={{
                  background: item.active ? '#F1F5F9' : 'rgba(15,26,43,.5)',
                  borderColor: '#E2E8F0',
                ...
                <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded shrink-0"
                  style={{ background: `${cat?.color ?? '#64748B'}18`, color: cat?.color ?? '#64748B' }}>
                  {cat?.label ?? item.category}
                </span>
                <p className="text-sm font-semibold flex-1 truncate" style={{ color: '#0F172A' }}>
                  {item.name}
                </p>
                <p className="text-sm font-bold font-mono shrink-0" style={{ color: item.active ? '#10B981' : '#64748B' }}>
                  {BRL(item.valueCents)}
                </p>
      ...
      {!adding && !editingId && (
        <button onClick={() => setAdding(true)}
          ...
          style={{ borderColor: '#CBD5E1', color: '#1D4ED8' }}>
...
function ExpenseForm({
  ...
  return (
    <div className="rounded-lg border-2 p-3 space-y-2"
      style={{ background: '#F1F5F9', borderColor: '#1D4ED8' }}>
      ...
      <div className="flex gap-2 items-center">
        <span className="text-xs shrink-0" style={{ color: '#475569' }}>R$</span>
