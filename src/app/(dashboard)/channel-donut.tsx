...
export function ChannelDonut({ breakdown }: { breakdown: ChannelItem[] }) {
  ...
  for (const b of breakdown) {
    const color = b.value == null ? '#5A7A9A' : channelColor(b.value)
    ...
    return (
      <div className="rounded-xl border p-6" style={{ background: '#111827', borderColor: '#1E2D45' }}>
        <div className="flex items-center gap-2 mb-4">
          <Store className="h-4 w-4" style={{ color: '#00E5FF' }} />
      ...
      {top && top.value != null && (
        <div className="mb-4 rounded-lg border p-3 flex items-center gap-2"
          ...
          <p className="text-xs" style={{ color: '#E8F0FE' }}>
            <strong style={{ color: '#00E5FF' }}>{channelLabel(top.value)}</strong> é o canal principal —{' '}
      ...
      <div className="flex items-center gap-6 flex-wrap">
        <div className="h-40 w-40 rounded-full shrink-0 relative" style={{ background: gradient }}>
          <div className="absolute inset-5 rounded-full flex flex-col items-center justify-center"
            ...
            <span className="text-[10px] uppercase tracking-wider" style={{ color: '#5A7A9A' }}>Total</span>
            <span className="text-base font-bold font-mono mt-0.5" style={{ color: '#E8F0FE' }}>
              {BRL(total)}
            </span>
          </div>
        </div>
            ...
            return (
              <div key={b.value ?? '__no__'} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span style={{ color: '#E8F0FE' }} className="truncate">{label}</span>
                </div>
                <div className="flex items-center gap-2 shrink-0 ml-2">
                  ...
                  <span className="font-mono font-semibold" style={{ color: '#8AA8C8' }}>
                    {BRL(b.totalCents)}
                  </span>
                </div>
              </div>
